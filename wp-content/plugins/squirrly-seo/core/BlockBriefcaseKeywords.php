    ...
    public function init() {
        ...
        if(SQ_Classes_Tools::getOption('sq_google_serp_active') == 0) {
            ...
            if (isset($this->checkin->active) && $this->checkin->active) {
                ...
                SQ_Classes_ObjController::getClass('SQ_Classes_Error')->setError(sprintf(__('%sStart a FREE Trial of the Business Plan with Advanced Analytics for 7 days. No credit card required. %sSee details%s %s'), '<strong style="font-size: 16px;">', '<a href="' . _SQ_DASH_URL_ . 'login/?token=' . SQ_Classes_Tools::getOption('sq_api') . '&redirect_to=' . _SQ_DASH_URL_ . 'user/plans?pid=31" target="_blank">', '</a>', '</strong>'), 'success');
            }
        }
